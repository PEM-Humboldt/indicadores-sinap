...
espec_mod <- stack(list.files("capas_base_ejemplos/BioModelos_N1/","tif",full.names=TRUE)) 

# 1.2 Cargar SINAP alias historico RUNAP a WGS84.

ap1990 <- shapefile("capas_base_ejemplos/RUNAP_shapefiles/RUNAP_1990.shp")
ap2010 <- shapefile("capas_base_ejemplos/RUNAP_shapefiles/RUNAP_2010.shp")
...
dir.create("productos/rep_ri/RUNAPRRi", showWarnings = FALSE)

# escribir shapefiles por tiempos
shapefile(x = spTransform(x = ap1990, CRSobj = crs() ), filename = "productos/rep_ri/RUNAPRRi/RUNAP_1990_rri.shp", overwrite = TRUE)
shapefile(x = ap2010, filename = "productos/rep_ri/RUNAPRRi/RUNAP_2010_rri.shp", overwrite = TRUE)
...
mapas_runap_ter <- list.files("productos/rep_ri/RUNAPRRi/", pattern="terr.dbf$", full.names=TRUE)
...
terr_2010<-aggregate(mapas_runap_ter[[2]]$RRi2010 ~ mapas_runap_ter[[2]]$nombre_1, data=mapas_runap_ter[[2]], FUN=sum)
