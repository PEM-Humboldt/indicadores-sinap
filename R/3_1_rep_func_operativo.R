...
# El mapa de huella humana a utilizar varia seg√∫n el a√±o a analizar, 2018 se usa como base para 2020
...
# "El tÈrmino de potencia escala el producto de dos valores de calidad. AquÌ,
...
# indicarÌan que el el valor combinado de dos celdas es desproporcionadamente 
# mayor para los pares de alta calidad que para los de baja calidad. Por el 
# contrario, z < 0,5 indicarÌa una penalizaciÛn desproporcionadamente mayor 
# para los pares de baja calidad" (Beyer et al, 2019)
#
# beta: integer, "El tÈrmino de penalizaciÛn de distancia beta determina cÛmo 
# el valor combinado de dos celdas disminuye a medida que funciÛn de la 
# distancia entre ellos (Fig. SM.2a). La elecciÛn de beta = 0,2 corresponde a 
# un 50% de penalizaciÛn a una separaciÛn de 5 km y una reducciÛn del 95% a 
# los 15 km, y se basÛ en una evaluaciÛn de cÛmo la mÈtrica de integridad 
# varÌa a travÈs de un conjunto de paisajes hipotÈticos que difieren en
# ·rea total del h·bitat, calidad y fragmentaciÛn" (Beyer et al, 2019)
#
# rad: integer, radio sobre el cual se crean zonas buffer por Area Protegida
# para el calculo de la integridad."En nuestra aplicaciÛn estas mÈtricas se 
# calculan utilizando un radio de 26,5 km, que capta el 99,5% de la 
# distribuciÛn exponencial (es decir, la contribuciÛn de las celdas m·s all· 
# de ese radio es insignificante pequeÒo debido al componente de ponderaciÛn
# de distancia exponencial de la funciÛn). Sin embargo, si el mÈtrica se
# implementa con valores alternativos del par·metro beta, que determina la 
# ponderaciÛn de la distancia, serÌa necesario ajustar el radio adecuado" 
# (Beyer et al, 2019). 
#
...
# integ_sd_total, vector numerico, desviaci√≥n de la integridad total del conjunto de areas protegidas 
# ingestadas (shp.areas)
...
# integ_raster, lista, lista de rasters con la informaci√≥n por pixel de integridad por area protegida
...
IntegAPS <- function(ras.Cal, shp.Areas, z = 0.5, beta = NULL, rad = 26.5, reescal = T){
  ...
  # No se configuro un par√°metro gamma para la transformaci√≥n de la huella humana.
  # Se uso una transformaci√≥n lineal para la capa de huella humana propuesta por 
  # Correa et al. (2020). Sin embargo, podr√≠a discutirse si se requiere una 
  # transformaci√≥n exponencial
  # gamma <- 0.2 # linea 108 para prender o apagar
  
  # En este loop se calculara el valor de integridad para cada √°rea
  ...
  for(p in 1:length(shp.Areas)){
    ...
    if(is.null(raster::intersect(extent(ras.Cal), (extent(shp.Areas[p,])+ 5000)))){
      ...
      # Ajustar dependiendo el tama√±o de celda y el tama√±o deseado del kernel
      #
      ...
      #
      # x = raster, medici√≥n de la calidad de h√°bitat 
      ...
      # Correr la m√©trica de integridad dentro de la ventana focal
      r.qp <- raster::focal(r.q, foc.w, fun = qprime, pad = TRUE, dvec = dvect)
      
      # plot(r.qp)
      
      # crop raster de integridad a la extensi√≥n del AP
...
# Establece el cambio en la integridad entre un par de a√±os consecutivos (t2 y t1). 
# En terminos simples, calcula t2 - t1 por area protegida en terminos de media y pixel en rasters.
# Deja afuera de la comparaci√≥n aquellas areas protegidas que no se habian creado en la temporalidad 2 (t2)

# Integ1: objeto integridad creado por la funci√≥n IntegAPS en el periodo 1
# Integ2: objeto integridad creado por la funci√≥n IntegAPS en el periodo 2
# tiempo1: vector character, de temporalidad del SINAP en el periodo 1. Por 
#ejemplo el a√±o el que representa las AP's 
...
# diferencia de integridad entre a√±os (indicador)
